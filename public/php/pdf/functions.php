...
function create_pdf_package($array,$template = 0){
	...
	if($array["driver_name1"])
		...
		$pfile = "Consortium_Enrollment_Renew_{$array["consortium_users_id"]}_".date("Y",$array["expiration_date"]).".pdf";
			...
			# for Linux:
$cmd = "/usr/local/bin/pdftk ".APPLICATION_PATH."/../public_html/pdf_templates/Enrollment_Package_Renewal.pdf
    fill_form
        ".APPLICATION_PATH."/../consortium_enrollment_pdf/data_{$array["consortium_users_id"]}.fdf
    output
        ".APPLICATION_PATH."/../consortium_enrollment_pdf/$pfile";
